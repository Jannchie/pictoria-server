...
class ColorThief:
    ...
    def __init__(self, file: str | PathLike | bytes | bytearray | Image.Image) -> None:
    ...
    def get_palette(self, color_count: int = 10, quality: int = 10):
...
class MMCQ:
    ...
    @staticmethod
    def quantize(pixels: list[tuple[int, int, int]], max_color: int) -> "CMap":  # noqa: C901
        """Quantize.
        ...
        if max_color < 2 or max_color > 256:  # noqa: PLR2004
        ...
        def iter_(lh: PQueue, target: int) -> None:
            n_color = 1
            n_iter = 0
...
class VBox:
    ...
    def contains(self, pixel: tuple[int, int, int]) -> bool:
        rval = pixel[0] >> MMCQ.RSHIFT
        gval = pixel[1] >> MMCQ.RSHIFT
        bval = pixel[2] >> MMCQ.RSHIFT
        return all(
...
class CMap:
    ...
    def nearest(self, color: tuple[int, int, int]) -> tuple[int, int, int]:
        d1 = None
        p_color = None
    ...
    def map(self, color: tuple[int, int, int]) -> tuple[int, int, int]:
        for i in range(self.vboxes.size()):
            vbox = self.vboxes.peek(i)
...
class PQueue:
    ...
    def push(self, o: dict):
        self.contents.append(o)
        self._sorted = False

    def peek(self, index: int | None = None):
        if not self._sorted:
            self.sort()
        if index is None:
